...
Codex `model_catalog_json` 要求的 `{"models":[...]}` 文件正文，直接落盘即可，不需要再加工。条目取自该账号
所属套餐的原生目录快照，保留上游的模型标识、展示名称、推理强度、上下文窗口与服务档位，不做别名改写或
字段裁剪；因此配置了模型别名时，客户端应使用网关对外的模型标识，导出的原生 slug 只反映上游目录。

目录正文只包含模型元数据，不含 access token、refresh token 或任何账号凭据。只有能提供 Codex 原生目录的
账号可以导出：xAI 账号与只返回模型 ID 的 API 目录不满足 `model_catalog_json` 的加载要求，返回 `400` 并
说明该账号没有可导出的原生目录，不降格成只含模型 ID 的文件。

客户端使用时把文件保存到本地任意可读路径，在 `config.toml` 写
`model_catalog_json = "/path/to/cpr-model-catalog-<账号>.json"`，重启客户端后才会重新加载目录。
