...
async fn unified_login_returns_server_identity_and_rotates_the_previous_session() {
    ...
    assert_eq!(
        get(&app, "/api/client/usage", &key_cookie).await.status(),
    ...
    assert_eq!(
        get(&app, "/api/client/usage", &renewed).await.status(),
        StatusCode::OK
    );
}
...
async fn valid_key_session_cannot_read_or_mutate_admin_resources() {
    ...
    assert_eq!(
        get(&app, "/api/client/usage", &cookie).await.status(),
        StatusCode::OK
    );
}
...
async fn admin_session_is_not_a_key_session() {
    ...
    for path in [
        ...
        "/api/client/usage/records?range=7d&endsAt=2026-09-14T00:00:00Z",
    ] {
        assert_eq!(
...
async fn declared_roles_and_foreign_key_ids_cannot_override_server_authority() {
    ...
    for suffix in [
        ...
        assert_eq!(
            get(&app, &format!("/api/client/usage?{suffix}"), &cookie)
                .await
                .status(),
...
async fn data_plane_keys_and_browser_sessions_are_not_interchangeable() {
    ...
    for name in [header::AUTHORIZATION.as_str(), "x-api-key"] {
        let request = Request::get("/api/client/usage")
    ...
    assert_eq!(
        get(&app, "/api/client/usage", &format!("cpr_session={RAW_KEY}"))
            .await
            .status(),
        StatusCode::UNAUTHORIZED
    );
...
async fn removed_auth_routes_do_not_fall_back_to_spa_or_accept_old_cookies() {
    ...
    for old_name in ["cpr_admin_session", "cpr_client_session"] {
        ...
        assert_eq!(
            get(&app, "/api/client/usage", &old_cookie).await.status(),
            StatusCode::UNAUTHORIZED
        );
    }
}
