...
async fn client_usage_should_require_a_session_and_expose_only_the_session_key_projection() {
    ...
    let missing = app
        .clone()
        .oneshot(empty_request(Method::GET, "/api/client/usage?range=7d"))
        .await
    ...
    assert_eq!(body["data"]["activity"]["range"]["kind"], "24h");
    assert_eq!(body["data"]["activity"]["totals"]["requestCount"], 7);
    assert_eq!(body["data"]["activity"]["totals"]["billedUsd"], "1.25");
    assert_eq!(
        body["data"]["limits"],
        json!({
            "maxConcurrency": 4,
...
impl ClientUsageStore for ClientRouteStore {
    async fn list_client_usage_records(
        &self,
        key_id: &ClientApiKeyId,
        query: gateway_admin::model::client_usage::ClientUsageQuery,
    ) -> AdminStoreResult<gateway_admin::model::client_usage::ClientUsageRecordsPage> {
        use gateway_admin::model::{
            client_usage::{ClientUsageRecord, ClientUsageRecordsPage},
        ...
        Ok(ClientUsageRecordsPage {
            items: vec![ClientUsageRecord {
                id: "request-42".to_owned(),
                started_at: query.range.start,
